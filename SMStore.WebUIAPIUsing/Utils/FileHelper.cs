namespace SMStore.WebUIAPIUsing.Utils
{
    public class FileHelper
    {
        public static async Task<string> FileLoaderAsync(IFormFile formFile, string filePath = "/wwwroot/Img/")
        {
            ...
            string directory = Directory.GetCurrentDirectory() + filePath + fileName; // dosyanın yükleneceği dizini belirledik(GetCurrentDirectory metodu uygulananın çalıştığı fiziksel yolu getirir)
        ...
        public static bool FileRemover(string fileName, string filePath = "/wwwroot/Img/")
        {
            ...
            if (File.Exists(directory)) // File.Exists metodu c# ta var olan ve kendisine verilen afreste dosya var mı yok mu kontrol eden metottur.
            {
